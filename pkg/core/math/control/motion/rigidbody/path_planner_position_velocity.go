...
type positionVelocityPlanner struct {
...
func newPositionVelocityPlanner(points []vec.Vector3D, hints []vec.Vector3D) (pathPlanner, error) {
	...
	planner := &positionVelocityPlanner{
		positions:   points,
		linearHints: hints,
		lengths:     lengths,
		total:       total,
		signature:   hashPositionsWithHints(points, hints),
	}
	return planner, nil
}

func (p *positionVelocityPlanner) Length() float32 {
	return p.total
}

func (p *positionVelocityPlanner) Signature() uint64 {
	return p.signature
}

func (p *positionVelocityPlanner) Sample(progress float32) samplePoint {
	if progress <= 0 {
	...
	hint := interpolateVector(p.linearHints[idx], p.linearHints[idx+1], ratio)
	return samplePoint{
		position:       pos,
		tangent:        tangent,
		orientation:    quaternionFromTangent(tangent),
...
func (p *positionVelocityPlanner) Project(position vec.Vector3D) float32 {
	best := float32(0)
...
func (p *positionVelocityPlanner) Curvature(progress float32) float32 {
...
func hashPositionsWithHints(points []vec.Vector3D, hints []vec.Vector3D) uint64 {
	h := uint64(1469598103934665603)
	for i := range points {
		h = hashVector(h, points[i])
		h = hashVector(h, hints[i])
	}
	return h
}
