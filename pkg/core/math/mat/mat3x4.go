...
var _ matTypes.Matrix = (*Matrix3x4)(nil)

func (m *Matrix3x4) view() Matrix {
	return Matrix{
		m[0][:],
		m[1][:],
		m[2][:],
...
func (m *Matrix3x4) IsContiguous() bool {
	return true
}
...
func (m *Matrix3x4) Matrix() matTypes.Matrix {
	return m.view()
}
...
func (m *Matrix3x4) Col(col int, v vecTypes.Vector) vecTypes.Vector {
...
func (m *Matrix3x4) Submatrix(row, col int, m1 matTypes.Matrix) matTypes.Matrix {
...
func (m *Matrix3x4) Clone() matTypes.Matrix {
	clone := &Matrix3x4{}
	for i, row := range m {
		copy(clone[i][:], row[:])
	}
...
func (m *Matrix3x4) MulVec(v vecTypes.Vector, dst vecTypes.Vector) vecTypes.Vector {
...
func (m *Matrix3x4) MulVecT(v vecTypes.Vector, dst vecTypes.Vector) vecTypes.Vector {
...
func (m *Matrix3x4) Cholesky(dst matTypes.Matrix) error {
	return m.view().Cholesky(dst)
}

func (m *Matrix3x4) CholeskySolve(b vecTypes.Vector, dst vecTypes.Vector) error {
	return m.view().CholeskySolve(b, dst)
}

func (m *Matrix3x4) QRDecompose(dst *matTypes.QRResult) error {
	return m.view().QRDecompose(dst)
}

func (m *Matrix3x4) QR(dst *matTypes.QRResult) error {
	return m.view().QR(dst)
}

func (m *Matrix3x4) Inverse(dst matTypes.Matrix) error {
	return m.view().Inverse(dst)
}

func (m *Matrix3x4) PseudoInverse(dst matTypes.Matrix) error {
	return m.view().PseudoInverse(dst)
}

func (m *Matrix3x4) DampedLeastSquares(lambda float32, dst matTypes.Matrix) error {
	return m.view().DampedLeastSquares(lambda, dst)
}

func (m *Matrix3x4) SVD(dst *matTypes.SVDResult) error {
...
func (m *Matrix3x4) GetCol(col int, dst vecTypes.Vector) vecTypes.Vector {
	return m.view().GetCol(col, dst)
}

func (m *Matrix3x4) Diagonal(dst vecTypes.Vector) vecTypes.Vector {
...
func (m *Matrix3x4) LU(L, U matTypes.Matrix) {
	m.view().LU(L, U)
}
