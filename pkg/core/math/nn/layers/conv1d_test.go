...
func TestNewConv1D(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			stride:         1,
			pad:            1,
			opts:           []Conv1DOption{WithConv1DBias(false)},
			expectError:    false,
			expectBias:     false,
			expectCanLearn: false,
		},
		{
			...
			stride:         1,
			pad:            1,
			opts:           []Conv1DOption{WithConv1DCanLearn(true)},
			expectError:    false,
			expectBias:     true,
			expectCanLearn: true,
		},
		{
			...
			stride:      1,
			pad:         1,
			opts:        []Conv1DOption{WithConv1DName("my_conv1d")},
			expectError: false,
			expectBias:  true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				weight := conv.Weight()
				assert.Equal(t, []int{tt.outChannels, tt.inChannels, tt.kernelLen}, weight.Dim, "Weight shape should match")

				if tt.expectBias {
					bias := conv.Bias()
					assert.Equal(t, []int{tt.outChannels}, bias.Dim, "Bias shape should match")
				} else {
					bias := conv.Bias()
...
func TestConv1D_Backward(t *testing.T) {
	...
	conv2, _ := NewConv1D(3, 16, 3, 1, 1, WithConv1DCanLearn(true))
	...
	assert.Equal(t, inputShape, gradInput2.Dim, "GradInput shape should match input shape")
...
func TestConv1D_Bias(t *testing.T) {
	...
	// Test without bias
	conv2, err := NewConv1D(3, 16, 3, 1, 1, WithConv1DBias(false))
	require.NoError(t, err, "Should create Conv1D layer")
...
func TestConv1D_SetWeight(t *testing.T) {
	...
	for i := range newWeight.Data {
		newWeight.Data[i] = float32(i) * 0.01
	}
	...
	// Test error cases
	err = conv.SetWeight(tensor.Tensor{Dim: []int{16, 4, 3}, Data: make([]float32, 192)})
...
func TestConv1D_SetBias(t *testing.T) {
	...
	// Test error cases
	err = conv.SetBias(tensor.Tensor{Dim: []int{17}, Data: make([]float32, 17)})
	assert.Error(t, err, "Should return error for wrong shape")
...
func TestConv1D_BackwardAccuracy(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			opts := []Conv1DOption{WithConv1DCanLearn(true)}
			if !tt.hasBias {
				opts = append(opts, WithConv1DBias(false))
			}
			...
			if tt.expectedWeightGrad != nil {
				weightParam := conv.Parameter(C1PARAM_WEIGHT)
			...
			if tt.hasBias && tt.expectedBiasGrad != nil {
				biasParam := conv.Parameter(C1PARAM_BIAS)
