...
func TestFlatten(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := flatten.Init(tt.input.Shape().ToSlice())
...
func TestUnsqueeze(t *testing.T) {
	t.Run("add_dim_at_beginning", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, []int{1, 3, 4}, output.Shape().ToSlice())
...
func TestSqueeze(t *testing.T) {
	t.Run("squeeze_all", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, []int{3, 4}, output.Shape().ToSlice())
		assert.Equal(t, input.Data(), output.Data())
	})
...
func TestTranspose(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, []int{3, 2}, output.Shape().ToSlice())
