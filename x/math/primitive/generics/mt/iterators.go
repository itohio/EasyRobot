...
// If you need to store the indices, copy them: `indicesCopy := make([]int, len(indices)); copy(indicesCopy, indices)`
//
// Note: In multithreaded build, this falls back to single-threaded implementation.
// Parallelization of iterators is complex due to Go's range-over-func contract requirements.
//
// Usage:
...
func ElementsIndices(shape []int, fixedAxisValuePairs ...int) func(func([]int) bool) {
	return st.ElementsIndices(shape, fixedAxisValuePairs...)
}
...
func ElementsIndicesStrided(shape []int, strides []int, dims ...int) func(func([]int) bool) {
	return st.ElementsIndicesStrided(shape, strides, dims...)
}
